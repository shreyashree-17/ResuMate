export { default as web1 } from "../img/courses/web/1.png";
export { default as web2 } from "../img/courses/web/2.png";
export { default as web3 } from "../img/courses/web/3.png";
export { default as web4 } from "../img/courses/web/4.png";


export { default as person1 } from "../img/users/person-1.png";
export { default as person2 } from "../img/users/person-2.png";
export { default as person3 } from "../img/users/person-3.png";
export { default as person4 } from "../img/users/person-4.png";
